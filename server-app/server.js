...
const recipesApi = require('./lib/recipesApi.js');
const recipeUtil = require('./lib/recipeUtil.js');
...
app.get('/', (req, res) => {
  return res.json(recipeUtil.status);
});
...
app.post('/api/resource', (req, res) => {
  ...
  cloudant
    .create(type, name, description, quantity, location, contact, unit, userID, id, ingredients, steps, imageUrl )
    .then(data => {
...
app.patch('/api/resource/:id', (req, res) => {
  ...
  const contact = req.body.contact || '';
  const unit = req.body.unit || '';
  const ingredients = req.body.ingredients || 'N/A';
  const steps = req.bodysteps || 'N/A';
  const imageUrl = req.body.imageUrl || 'N/A';
  cloudant
    .update(req.params.id, type, name, description, quantity, location, contact, unit, userID, id, ingredients, steps, imageUrl )
    .then(data => {
      if (data.statusCode != 200) {
        res.sendStatus(data.statusCode)
      } else {
...
// Get repipes optimal list from Rapid API
//
app.get('/api/recipes', (req, res) => {
  var type = req.query.type  || false;
  var name = req.query.name || "chicken"; 
  var userID = req.query.userID || false;
  try{
    console.log("name->> " + name);
    var rest = recipesApi.sendRequest(req, res, recipesApi.options(recipesApi.url));
  } catch(err){
    handleError(res, err);
  } 
});

// Get repipes full list from Rapid API
//
app.get('/api/completerecipes', (req, res) => {
  var type = req.query.type  || false;
  var name = req.query.name || "chicken"; 
  var userID = req.query.userID || false;
  try{
